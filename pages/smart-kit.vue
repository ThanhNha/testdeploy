<template>
  <main>
    <h1></h1>
    <JBanner
      img="/images/smart-kit/banner.jpg"
      className="h-[30vh] md:h-[calc(70vh-var(--header-height-md))] xl:h-[calc(100vh-var(--header-height-lg))]"
    />

    <section
      class="bg-gray-100 xl:pt-[72px] md:pt-[50px] pt-[36px] xl:pb-[64px] md:pb-[45px] pb-[32px]"
    >
      <div class="container">
        <div class="flex flex-col xl:space-y-[34px] md:space-y-[24px] space-y-[17px]">
          <AnimateAppear v-for="(item, index) in comboKit" :key="index" :delay="index * 300">
            <div class="xl:p-[32px] md:p-[22px] p-[16px] bg-white rounded-[20px]">
              <div class="grid grid-cols-12 xl:gap-x-[32px] md:gap-x-[22px] gap-y-[16px]">
                <div
                  :class="index % 2 === 0 ? 'order-1 md:order-2' : ' order-1'"
                  class="col-span-full md:col-span-6 lg:col-span-7"
                >
                  <div class="aspect-w-8 aspect-h-5 rounded-[20px] overflow-hidden">
                    <JPicture
                      :src="item.image_url || '/images/cover-combo.jpg'"
                      class="picture-cover image"
                    />
                  </div>
                </div>
                <div
                  class="flex flex-col items-start justify-center md:items-center col-span-full md:col-span-6 lg:col-span-5"
                  :class="index % 2 === 0 ? 'order-2 md:order-1' : ' order-2'"
                >
                  <div class="flex flex-col items-start w-full md:w-auto">
                    <div>
                      <div class="font-bold uppercase h2 text-linear-2">
                        {{ item.name }}
                      </div>
                      <div class="bg-line-green w-full h-[6px]"></div>
                    </div>
                    <div
                      class="h4 font-bold text-blue-50 xl:mt-[24px] md:mt-[17px] mt-[12px] xl:mb-[32px] md:mb-[22px] mb-[16px]"
                    >
                      Giá: {{ item.price_smart | formatPrice }}
                    </div>
                    <div
                      class="list ml-[24px] font-medium text-black description_combo"
                      v-html="item.description"
                    ></div>
                    <div
                      class="xl:mt-[32px] md:mt-[22px] mt-[16px] flex items-center space-x-[16px] md:w-auto w-full"
                    >
                      <JButton
                        title="Xem chi tiết"
                        variant="secondary"
                        @click.native="getCurrentCombo(item)"
                        className="w-full md:w-auto"
                      />

                      <JButton
                        title="Mua bộ kit"
                        className="w-full md:w-auto"
                        :href="`https://rangdongstore.vn/don-hang?combo_id=${item.id}`"
                      />
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </AnimateAppear>
        </div>
      </div>
    </section>

    <JModal :show="currentModal === 'sampleModal'" @close="currentModal = null">
      <div>
        <div class="font-bold text-center uppercase h3 text-linear-2">
          {{ currentCombo.name }}
        </div>
        <div class="my-[10px] md:my-[12px] xl:my-[24px]">
          <div class="md:grid md:grid-cols-2 lg:grid-cols-5 gap-[8px]">
            <div
              class="md:p-[8px] bg-white rounded-[16px] grid grid-cols-5 lg:flex lg:flex-col gap-x-[8px] mb-[10px] md:mb-0 last:mb-0"
              v-for="(product, index) in currentCombo.products"
              :key="index"
            >
              <div class="col-span-2 lg:col-span-full">
                <div class="rounded-[16px] overflow-hidden">
                  <div class="aspect-w-1 aspect-h-1">
                    <JPicture
                      :src="product.image_url || '/images/cover-combo-square.jpg'"
                      class="picture-cover"
                    />
                  </div>
                </div>
              </div>
              <div class="lg:mt-[8px] lg:py-[16px] col-span-3 lg:col-span-full">
                <div class="font-semibold text-gray-800 lg:text-center line-clamp-2">
                  {{ product.title }}
                </div>
                <div class="text-gray-600 lg:text-center line-clamp-2">
                  Model: {{ product.sku }}
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="text-center xl:mb-[24px] md:mb-[17px] my-[12px]">
          <div class="p1 text-blue-50">Giá: {{ currentCombo.price_smart | formatPrice }}</div>
        </div>
        <div class="flex items-center justify-center">
          <JButton
            title="Mua combo tại rạng đông store"
            icon="DoubleRight"
            :href="`https://rangdongstore.vn/don-hang?combo_id=${currentCombo.id}`"
            class="hidden md:block"
          />
          <JButton
            title="Mua combo tại rạng đông store"
            :href="`https://rangdongstore.vn/don-hang?combo_id=${currentCombo.id}`"
            class="block md:hidden"
          />
        </div>
      </div>
    </JModal>

    <SocialMeta
      title="Smart Kit - Giải pháp hoàn hảo để trải nghiệm smart lighting"
      description="RalliSmart tạo ra Smart Kit - những gói sản phẩm đơn giản, tiết kiệm, giúp khách hàng có thể trải nghiệm chiếu sáng thông minh một cách dễ dàng."
    />
  </main>
</template>

<script>
export default {
  data() {
    return {
      currentModal: null,
      currentCombo: [],
      comboKit: [],
    }
  },
  async fetch() {
    const { data: comboKit } = await this.$api.product.comboFilter({
      category_id: 2,
    })
    this.comboKit = comboKit
  },
  methods: {
    getCurrentCombo(obj) {
      this.currentModal = 'sampleModal'
      this.currentCombo = obj
      if ((this.currentModal = 'sampleModal')) {
        document.body.style.overflowY = 'hidden'
        document.body.style.height = '100vh'
      }
    },
  },
}
</script>

<style lang="scss" scoped>
ul {
  list-style: none;
  li {
    @apply relative mb-[4px] lg:mb-[8px] last:mb-0;
  }

  li:before {
    position: absolute;
    @apply w-[12px] h-[12px] top-[5px] rounded-full border border-white bg-green-50 left-[-24px];
    vertical-align: middle;
    display: inline-block;
    content: '';
    box-sizing: border-box;
    box-shadow: 0px 0px 10px rgba(4, 216, 155, 0.7);
  }
}

::v-deep {
  .description_combo {
    p {
      @apply relative mb-[4px] lg:mb-[8px] last:mb-0;
    }

    p:before {
      position: absolute;
      @apply w-[12px] h-[12px] top-[5px] rounded-full border border-white bg-green-50 left-[-24px];
      vertical-align: middle;
      display: inline-block;
      content: '';
      box-sizing: border-box;
      box-shadow: 0px 0px 10px rgba(4, 216, 155, 0.7);
    }
  }
}
</style>
